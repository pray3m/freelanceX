...
class SearchService {
  ...
  async fuzzySearch(searchTerm, category) {
    ...
    const sortedResults = rankedResults.sort((a, b) => b.relevanceScore - a.relevanceScore);

    searchLog(`🏆 Search completed: ${sortedResults.length} results for "${searchTerm}"`);
    sortedResults.slice(0, 3).forEach((gig, index) => {
      searchLog(
        `  ${index + 1}. "${gig.title.substring(0, 40)}..." - Score: ${gig.relevanceScore}`
      );
    });
  ...
  calculateQualityScore(gig) {
    ...
    const daysSinceCreation = (new Date() - new Date(gig.createdAt)) / (1000 * 60 * 60 * 24);
